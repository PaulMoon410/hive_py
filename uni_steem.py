...
TRADE_HISTORY_FILE = os.path.join(os.path.dirname(__file__), ".steem_trade_history.json")
...
TOKEN = "SWAP.STEEM"
ENABLE_STEEM_BOT = os.environ.get("ENABLE_STEEM_BOT", "false").lower() in ("1", "true", "yes")
DELAY = 1500
...
def smart_trade(account_name, token):
        ...
        if one_percent_hive > 0:
            ...
            if doge_bid > 0:
                ...
                try:
                    ...
                    print(f"[STEEM BOT] Aggressive market buy: {market_buy_qty} SWAP.DOGE at {doge_bid} (1% of SWAP.HIVE)")
                except Exception as e:
                    print(f"[STEEM BOT] Aggressive market buy exception: {e}")
            else:
                print(f"[STEEM BOT] Could not fetch SWAP.DOGE bid for 1% buy.")
    print("\n==============================")
    print(f"[STEEM BOT] Starting Smart Trade for {token}")
    try:
        success = buy_matic_gas(account_name, active_key=HIVE_ACTIVE_KEY, nodes=HIVE_NODES)
        if success:
            print("[STEEM BOT] SWAP.DOGE gas buy submitted (cycle-start).")
        else:
            print("[STEEM BOT] SWAP.DOGE gas buy skipped or failed (cycle-start).")
    except Exception as e:
        print(f"[STEEM BOT] SWAP.DOGE buy exception (cycle-start): {e}")

    rc_percent = get_resource_credits(account_name)
    if rc_percent is not None:
        print(f"[STEEM BOT] Resource Credits: {rc_percent}%")
        if rc_percent < 10.0:
            LOW_RC_CONSECUTIVE_COUNT += 1
            DYNAMIC_DELAY = max(DELAY, DYNAMIC_DELAY * 2)
            print(f"[STEEM BOT] WARNING: Resource Credits too low ({rc_percent}%). Skipping trade cycle.")
            print(f"[STEEM BOT] Low RC count: {LOW_RC_CONSECUTIVE_COUNT}. Next delay: {DYNAMIC_DELAY}s ({DYNAMIC_DELAY/60:.1f} min)")
            print("==============================\n")
            return
        else:
            if DYNAMIC_DELAY > DELAY:
                ...
                print(f"[STEEM BOT] RC recovered! Reducing delay to {DYNAMIC_DELAY}s ({DYNAMIC_DELAY/60:.1f} min)")
    else:
        print(f"[STEEM BOT] Resource Credits: Unable to fetch.")
    ...
    print(f"[STEEM BOT] Current total open orders (from orderbook): {open_count}")
    
    # If severely maxed out (200+), cancel multiple oldest orders
    if open_count >= 200:
        print(f"[STEEM BOT] 🚨 CRITICAL: Account maxed out with {open_count} orders! Cancelling 3 oldest orders...")
        for i in range(3):
            try:
                cancel_oldest_order(account_name, None, active_key=HIVE_ACTIVE_KEY, nodes=HIVE_NODES)
                print(f"[STEEM BOT] Cancelled oldest order {i+1}/3")
                time.sleep(2)
            except Exception as e:
                print(f"[STEEM BOT] Failed to cancel order {i+1}: {e}")
        print(f"[STEEM BOT] Skipping trade cycle to allow orders to clear.")
        print("==============================\n")
        return
    
    # If approaching limit, cancel one oldest order for THIS token, but continue trading
    if open_count >= 190:
        print(f"[STEEM BOT] Open orders at {open_count}. Cancelling oldest {token} order to self-regulate.")
        try:
            cancel_oldest_order(account_name, token, active_key=HIVE_ACTIVE_KEY, nodes=HIVE_NODES)
            time.sleep(2)
        except Exception as e:
            print(f"[STEEM BOT] Failed to cancel oldest {token} order: {e}")
    else:
        print(f"[STEEM BOT] Open orders at {open_count}. Below 190 threshold, proceeding with normal trading.")

    market = get_orderbook_top(token)
    if not market:
        print(f"[STEEM BOT] Market fetch failed for {token}. Skipping this cycle.")
        print("==============================\n")
        return

    print(f"[STEEM BOT] Market fetch success for {token}.")
    bid = float(market.get("highestBid", 0))
    ask = float(market.get("lowestAsk", 0))
    buy_price = round(bid * 0.95, 6) if bid > 0 else 0  # Buy at 5% below highest bid
    ...
    steem_balance = get_balance(account_name, token)
    print(f"[STEEM BOT] HIVE balance: {hive_balance}")
    print(f"[STEEM BOT] STEEM balance: {steem_balance}")
    print(f"[STEEM BOT] Buy price: {buy_price}")
    print(f"[STEEM BOT] Success rate: {success_rate*100:.1f}% | Adaptive floor: {adaptive_floor*100:.3f}% | Dynamic profit target: {min_profit_percent * 100:.3f}%")

    buy_qty = round(hive_balance * 0.20 / buy_price, 6) if buy_price > 0 else 0
    sell_qty = round(steem_balance * 0.30, 6)

    print(f"[STEEM BOT] Preparing BUY: {buy_qty} {token} at {buy_price}")
    print(f"[STEEM BOT] Preparing SELL: {sell_qty} {token} at {sell_price}")
    ...
    if buy_qty <= 0:
        print(f"[STEEM BOT] Skipping BUY: buy_qty is zero or negative. Check HIVE balance and buy price.")
    elif duplicate_buy:
        print(f"[STEEM BOT] Skipping BUY: Duplicate buy order at {buy_price} detected.")
    else:
        try:
            ...
            print(f"[STEEM BOT] BUY order submitted: {buy_qty} {token} at {buy_price}")
            track_trade_attempt(account_name, token, filled=True)
            time.sleep(5)
            open_orders = get_open_orders(account_name, token)
            if open_orders:
                print(f"[STEEM BOT] Open orders after BUY: {len(open_orders)} found.")
            else:
                print(f"[STEEM BOT] No open orders found after BUY (may be node delay).")
            time.sleep(1)
        except Exception as e:
            print(f"[STEEM BOT] BUY order exception: {e}")
    ...
    if steem_balance > 0:
        ...
        if duplicate_sell:
            print(f"[STEEM BOT] Skipping SELL: Duplicate sell order at {sell_price} detected.")
            ...
            try:
                ...
                print(f"[STEEM BOT] SELL order submitted: {sell_qty} {token} at {sell_price}")
                print(f"[STEEM BOT] Profit percent: {get_profit_percent(buy_price, sell_price)}%")
                track_trade_attempt(account_name, token, filled=True)
                time.sleep(5)
                open_orders = get_open_orders(account_name, token)
                if open_orders:
                    print(f"[STEEM BOT] Open orders after SELL: {len(open_orders)} found.")
                else:
                    print(f"[STEEM BOT] No open orders found after SELL (may be node delay).")
                time.sleep(1)
            except Exception as e:
                print(f"[STEEM BOT] SELL order exception: {e}")
        elif sell_price > 0 and sell_qty > 0:
            print(f"[STEEM BOT] SELL order skipped: Not profitable (sell {sell_price} <= last buy {get_last_buy_price()})")
        else:
            print(f"[STEEM BOT] SELL order skipped: sell_qty is zero.")
    else:
        print(f"[STEEM BOT] SELL order skipped: steem_balance is zero.")
    ...
    try:
        if ask > 0:
            ...
            print(f"[STEEM BOT] {token} tiny buy attempted at {ask}.")
        else:
            print(f"[STEEM BOT] {token} market ask unavailable, skipping tiny buy.")
    except Exception as e:
        print(f"[STEEM BOT] {token} tiny buy exception: {e}")
    
    # Always buy smallest increment of SWAP.DOGE at fixed tiny price
    try:
        success = buy_matic_gas(account_name, active_key=HIVE_ACTIVE_KEY, nodes=HIVE_NODES)
        if success:
            print("[STEEM BOT] SWAP.DOGE gas buy submitted.")
        else:
            print("[STEEM BOT] SWAP.DOGE gas buy skipped or failed.")
    except Exception as e:
        print(f"[STEEM BOT] SWAP.DOGE buy exception: {e}")
    ...
    try:
        ...
        print("[STEEM BOT] PEK gas buy attempted via helper.")
    except Exception as e:
        print(f"[STEEM BOT] PEK gas buy exception: {e}")

    print(f"[STEEM BOT] Trade cycle for {token} complete.")
    print("==============================\n")


if __name__ == "__main__":
    if not ENABLE_STEEM_BOT:
        print("[STEEM BOT] Disabled by config (set ENABLE_STEEM_BOT=true to override). Exiting.")
        raise SystemExit(0)
